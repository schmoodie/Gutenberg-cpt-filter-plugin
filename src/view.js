...
function handleOptionChange(event) {
  ...
  const selectedValues = Array.from(event.target.selectedOptions)
    .filter((option) => option.value.toLowerCase() !== 'visa alla')
  ...
  if (hasChanged) {
    ...
    Array.from(filterPosts).forEach((div) => {
      ...
      const shouldShow = Object.values(selectedOptions).every((options) =>
        options.every((option) => postClasses.includes(option))
      );
...
function parseFilterFromUrl() {
  ...
  Array.from(filterPosts).forEach((div) => {
    ...
    if (filter) {
      ...
      // Check if the div contains all the selected classes from the URL filter
      const shouldShow = filterValues.every((selectedClass) =>
        postClasses.includes(selectedClass)
      );
